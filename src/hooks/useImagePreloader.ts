...
export function useImagePreloader(assets: Asset[], currentAssetId: string, preloadCount = 2) {
  ...
  const preloadImage = useCallback(
    (assetId: string) => {
      if (!assetId || preloadedImages.has(assetId)) {
        return
      }
      ...
      thumbnailImg.onload = () => {
        setLoadingStatus((prev) => ({
          ...prev,
          [assetId]: { fullImageLoaded: false, ...prev[assetId], thumbnailLoaded: true },
        }))
        ...
        fullImg.onload = () => {
          setLoadingStatus((prev) => ({
            ...prev,
            [assetId]: { thumbnailLoaded: false, ...prev[assetId], fullImageLoaded: true },
          }))
        }
        fullImg.src = fullUrl
      }
  ...
  }, [currentAssetId, currentIndex, assets, preloadCount, preloadImage])
