...
export function useZoomTransition({
  ...
  // Animation function
  const startZoomAnimation = (startProgress: number, endProgress: number, params: { offsetY: number }) => {
  ...
  }, [isOpen, thumbnailPosition, isAnimating, isZoomingIn, isZoomingOut]);

  // Start zoom-out animation
  const startZoomOut = (options: { offsetY?: number } = {}) => {
    if (!thumbnailPosition) return;
  ...
  const getImageTransform = () => {
    ...
    const position = isZoomingOut ? thumbnailPosition : (initialPositionRef.current || thumbnailPosition);
