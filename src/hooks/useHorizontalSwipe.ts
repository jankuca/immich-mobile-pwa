...
export function useHorizontalSwipe({
  ...
  const handleTouchMove = (e: TouchEvent) => {
    ...
    if (direction === 'horizontal') {
      ...
      if (addEdgeResistance && 
          ((currentIndex === 0 && diffX > 0) || 
           (currentIndex === totalItems - 1 && diffX < 0))) {
  ...
  const handleTouchEnd = () => {
    ...
    if (isVeryFastSwipe()) {
      effectiveThreshold = windowWidth * veryFastSwipeThreshold;
    } else if (isFastSwipe()) {
      effectiveThreshold = windowWidth * fastSwipeThreshold;
    } else {
      effectiveThreshold = windowWidth * positionThreshold;
    }

    // Determine if we should navigate to the next/previous item
    if ((swipeOffset < -effectiveThreshold || 
    ...
    else if ((swipeOffset > effectiveThreshold || 
             (swipeVelocity > 0.3 && swipeOffset > 0) || 
             (swipeVelocity > 0.8 && swipeOffset > 0)) && 
             currentIndex > 0) {
  ...
  const getMainTransform = () => {
    if (!isHorizontalSwiping) return '';
    return `translateX(${swipeOffset}px)`;
  };

  const getPrevTransform = () => {
    if (!isHorizontalSwiping) return 'translateX(-100%)';
